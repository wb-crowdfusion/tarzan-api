	...
	public function authenticate($bucket, $opt = null, $location = null, $redirects = 0)
	{
		...
		else
		{
			...
			else
			{
			    ...
			    /*
			     * NOT prepending bucket to s3 domain so SSL communication
			     * works properly.
			     */
				$hostname = $this->hostname;
			}
			...
			else
			{
                ...
                /*
                 * If the bucket name is not already in the request (uri) then
                 * construct an SSL safe url so curl can communicate with s3's
                 * wildcard cert.  see README and ticket #166 for more info.
                 */
				if (!empty($filename) && false === strpos($request, $bucket)) {
				    $this->request_url = $scheme . $hostname . '/' . $bucket . '/' . ltrim($request, '/');
				} else {
