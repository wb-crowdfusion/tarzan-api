 ...
 * (end)
 */
class AmazonCloudFront extends TarzanCore
{
	/**
	 * Property: base_xml
	...
	public function authenticate($method = HTTP_GET, $path = null, $opt = null, $xml = null, $etag = null)
	{
		...
		$canonical_date = gmdate(DATE_AWS_RFC2616);
	 ...
	 * 	public
 	 *
	 * Parameters:
	 * 	xml - _SimpleXMLElement_|_TarzanHTTPResponse_|_string_ (Required) The source DistributionConfig XML to make updates to. Can be the SimpleXMLElement body of a <get_distribution_config()> response, the entire <TarzanHTTPResponse> of a <get_distribution_config()> response, or a string of XML generated by <generate_config_xml()> or <update_config_xml()>.
 	 * 	opt - _array_ (Optional) Associative array of parameters which can have the following keys:
 	 *
 	 * Keys for the $opt parameter:
	...
	{
		// If we receive a full TarzanHTTPResponse object, only use the body.
		if ($xml instanceof TarzanHTTPResponse)
		{
			$xml = $xml->body;
		}
